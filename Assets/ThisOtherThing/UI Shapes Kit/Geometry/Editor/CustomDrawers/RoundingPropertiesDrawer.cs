	...
	public override void OnGUI (Rect position, SerializedProperty property, GUIContent label)
	{
		...
		switch (resolutionType)
		{
			case RoundingProperties.ResolutionType.Calculated:
				EditorGUI.LabelField(propertyPosition, "Max Distance");

				propertyPosition.x += EditorGUIUtility.singleLineHeight * 6.0f;

				propertyPosition.width = position.width * 0.5f;
				EditorGUI.PropertyField(
					propertyPosition,
					property.FindPropertyRelative("ResolutionMaxDistance"),
					GUIContent.none
				);
				propertyPosition.width = position.width;
				break;
